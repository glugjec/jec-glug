...
const TeamManager = () => {
  ...
  const [confirmModal, setConfirmModal] = useState({ isOpen: false, type: '', data: null });
  const [formData, setFormData] = useState({
    name: '', position: '', role: 'CLUB HEAD', description: '', imageUrl: '', linkedinUrl: '', instagramUrl: ''
  });
  const [selectedImage, setSelectedImage] = useState(null);
  const [imagePreview, setImagePreview] = useState(null);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!editingMember && !selectedImage) return alert('Please select a profile image');
    if (editingMember && !selectedImage && !imagePreview) return alert('Please select a profile image');
  ...
  const handleConfirmSave = async () => {
    ...
    formDataToSend.append('insta', formData.instagramUrl || '');
    formDataToSend.append('linkedin', formData.linkedinUrl || '');
  
    const roleValue = Array.isArray(formData.role)
      ? formData.role.join(',')
      : (formData.role || '');
    ...
    try {
      const isEdit = editingMember && editingMember._id?.length === 24;
      let response = null;
      let result = null;
  
      if (isEdit) {
        // PATCH existing member
        response = await fetch(`${API_BASE}/members/${editingMember._id}`, {
          method: 'PATCH',
          headers: {
        ...
        const updatedMember = {
          ...
          imageUrl: `${API_BASE}/members/${editingMember._id}/image`
        };
  
        setMembers(prev =>
          prev.map(m => (m._id === editingMember._id ? updatedMember : m))
        );
      } else {
        // POST new member
        response = await fetch(`${API_BASE}/members`, {
          method: 'POST',
          headers: {
            'x-api-key': import.meta.env.VITE_API_KEY
          },
        ...
        const created = result.member || result; // fallback if result is directly the member
        const newMember = {
          ...created,
          imageUrl: `${API_BASE}/members/${created._id}/image`
        };
  ...
  const resetForm = () => {
    setFormData({ name: '', position: '', role: 'CLUB HEAD', description: '', imageUrl: '', linkedinUrl: '', instagramUrl: '' });
    setSelectedImage(null);
    setImagePreview(null);
    setEditingMember(null);
    setShowAddForm(false);
  ...
  const handleEdit = (member) => {
    ...
    setFormData({
      ...
      description: member.description || '',
      instagramUrl: member.instagramUrl || member.insta || '',
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-6">
        {members.map((member) => (
          <div key={member._id} className="bg-white/10 backdrop-blur-xl rounded-2xl p-4 sm:p-6 border border-white/20">
            <div className="text-center mb-4">
              <img
                src={member.imageUrl}
                alt={member.name}
