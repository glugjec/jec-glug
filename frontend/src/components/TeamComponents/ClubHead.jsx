...
const ClubHead = () => {
  ...
  const member = {
    ...
    role: "President",
    desc: "Leading GLUG into the future with passion for open source",
  };
...
const Section = ({ title, show, setShow, cols, members }) => (
  <div className="mx-auto my-8 max-w-5xl bg-white/10 backdrop-blur-lg rounded-2xl shadow-xl p-8 border border-white/20 transition-transform hover:scale-[1.01]">
...
const Card = ({ m }) => {
  ...
  return (
    <div className="bg-black/10 backdrop-blur-md rounded-lg p-4 flex items-center space-x-4 shadow">
      <img
        src={imageSrc}
        alt={m.name}
        onError={(e) => {
